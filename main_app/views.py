...
def home(request):
    ...
    new_sort = sorted(post_list, key=attrgetter('like_count'), reverse=True)
    # infiniscroll test
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if account_pic:
            ...
            key = uuid.uuid4().hex[:6] + \
                account_pic.name[account_pic.name.rfind('.'):]
            print("key test", key)
...
class PostCreate(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form, *args, **kwargs, ):
        ...
        form = Post(title=self.request.POST.get('title'), text_content=self.request.POST.get(
            'text_content'), tags=self.request.POST.get('tags'))
...
class CommentCreate(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form, *args, **kwargs, ):
        # print("POST Test",self.request.POST)
        form = Comment(title=self.request.POST.get('title'),
                       text_content=self.request.POST.get('text_content'))
        form.user_id = self.request.user.id
...
class CommentDelete(LoginRequiredMixin, DeleteView):
    ...
    def get_success_url(self):
        ...
        return self.request.GET.get('next', reverse('posts_index_likes'))
...
def SearchPost(request):
    if request.method == "POST":
        ...
        posts = Post.objects.filter(
            Q(tags__icontains=searched) | Q(title__icontains=searched))
    ...
    if request.method == 'POST':
        ...
        if account_pic:
            ...
            try:
                ...
                account_form = AccountCreate(
                    request.POST or None, request.FILES or None, instance=account_instance)
                if form.is_valid():
                    print("accountForm", account_form)
                    # save user to DB
                    user = form.save()
